    ...
    <div className="max-w-6xl mx-auto pb-16 px-4">
      ...
      <div className="mb-10 text-center lg:text-left">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-lg max-w-2xl">
          Upload an image to perform high-fidelity AI detection utilizing our local deep learning models combined with Gemini API multimodal reasoning.
        </p>
      </div>
          ...
          >
            ...
            {preview ? (
              ...
              <div className="flex flex-col items-center pointer-events-none select-none">
                ...
                <p className="text-sm text-slate-400 dark:text-slate-500">JPEG, PNG, WEBP — max 10MB</p>
              </div>
            )}
          </div>
          ...
          <div className="p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between transition-colors">
            <div className="flex flex-col gap-1 pr-4">
              ...
              <span className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed">
                Send the image to Gemini multimodal API to double check predictions against visual artifacts.
              </span>
            </div>
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          {result ? (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-500">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-md border border-slate-200 dark:border-slate-800 overflow-hidden">
                ...
                >
                  ...
                  >
                    {result.agreement_status === "agree" ? "✓ Systems Agree" :
                     result.agreement_status === "disagree" ? "⚠ Systems Disagree" :
                     "ℹ Gemini Unavailable"}
                  </span>
                </div>
                
                <div className="p-6 bg-slate-50/30 dark:bg-slate-900/30">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-350 mb-1">Recommendation:</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-md border border-slate-200 dark:border-slate-800 p-6 flex flex-col">
                  <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-100 dark:border-slate-800">
                    <h3 className="font-bold text-slate-800 dark:text-slate-200">Local PyTorch Model</h3>
                  ...
                  <div className="text-center mb-5">
                    <span className="text-xs text-slate-400 dark:text-slate-500 block mb-0.5">Confidence</span>
                  ...
                  <div className="space-y-3.5 flex-grow">
                    {[
                      { label: "AI generated (Fake)", val: result.local_model.fake_probability, color: "bg-red-500" },
                      { label: "Authentic (Real)", val: result.local_model.real_probability, color: "bg-green-500" }
                    ].map((bar) => (
                      <div key={bar.label}>
                  ...
                  <div className="pt-4 mt-5 border-t border-slate-100 dark:border-slate-800 space-y-1.5 text-xs text-slate-400 dark:text-slate-500">
                    <div className="flex justify-between"><span>Model:</span><span className="font-mono">{result.local_model.model_name}</span></div>
                    <div className="flex justify-between"><span>Latency:</span><span>{result.local_model.processing_time_ms} ms</span></div>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-md border border-slate-200 dark:border-slate-800 p-6 flex flex-col">
                  <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-100 dark:border-slate-800">
                    <h3 className="font-bold text-slate-800 dark:text-slate-200">Gemini Second Opinion</h3>
                  ...
                  {result.gemini_analysis && !result.gemini_analysis.error ? (
                    ...
                    <div className="flex-grow flex flex-col justify-center items-center text-center p-4">
                      ...
                      <p className="text-xs text-slate-450 dark:text-slate-500 leading-relaxed">
                        {result.gemini_analysis?.reasoning_summary || "Gemini analysis is unavailable or was disabled for this run."}
                      </p>
                    </div>
                  )}
                </div>
              ...
              {result.image_url && (
                <div className="flex justify-between items-center text-xs text-slate-400 dark:text-slate-500 px-2">
                  <a href={result.image_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-blue-500 hover:underline">
                    ...
                    View saved image on Cloudinary
                  </a>
            ...
            <div className="bg-slate-50 dark:bg-slate-900/20 rounded-2xl border border-slate-200 dark:border-slate-800 min-h-[440px] flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 p-8 text-center select-none">
              ...
              <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-350 mb-1">Awaiting Image</h3>
              <p className="text-sm text-slate-400 max-w-sm">
                Select an image on the left, choose if you want a Gemini second opinion, and click Analyze to view full hybrid statistics.
              </p>
            </div>
          )}
        </div>
